...
10. [サービス方針・運用上の注意事項](#10-サービス方針運用上の注意事項)
...
|---|---|
| 現状 | 縮小・終了の方向で運用中 |
| 縮小の理由 | 林野火災注意報の運用頻度増加／需要が少ない |
| 完全終了時期 | 未定（決定時に本書を更新） |

#### コンテンツ・SEO上の取扱い

- 新規ページ・記事・LP・お知らせで「直火」を差別化軸として打ち出さない
- ディープフォレストの訴求軸は「静寂・ブッシュクラフト・焚き火・上級者向け・18禁」とし、直火を主役にしない
- SEOキーワード戦略でも「直火 キャンプ場」系は対象外
- 既存ページに「直火」の表記が残っている場合は段階的に削除・置き換え

#### 既存ページでの直火表記の整理対象（要スキャン）
